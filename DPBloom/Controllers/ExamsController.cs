...
public class ExamsController : ControllerBase
{
    ...
    [HttpGet("update-details/{id:guid}")]
    [Authorize(Roles = "Teacher, Admin")]
    public async Task<ActionResult<ExamDetailsForUpdateDto>> GetByWithUpdateDetailsId(Guid id)
    {
        var exam = await _examService.GetExamUpdateDetailsAsync(id);
        if (exam == null) return NotFound();
    ...
    [Authorize(Policy = "RequireTeacherPrivileges")]
    public async Task<ActionResult<BloomLevel>> PredictBloomLevel([FromBody] PredictBloomRequestDto request)
    {
        var level = await _bloomPredictor.PredictLevelAsync(request.QuestionText);
